...
const app = createApp( {
  ...
  created (){
    ...
    .then ( data => {
      ...
      this.arrayCategory =  Array.from(new Set (this.eventosPasados.map(evento=>evento.category)))
    ...
    computed:{
        ...
        filtros (){
         ...
         this.filterCategory = this.eventosPasados.filter (events => this.categoryCheckbox.includes(events.category)||this.categoryCheckbox.length == 0)
